...
class MainViewModel @Inject constructor(val retrofitBuilder: RetrofitBuilder) : ViewModel() {
    ...
    val rateLD = MutableLiveData<ExchangeRateResponse>()
    ...
    private suspend fun getCurrencyStringList(exchangeRateResponse: ExchangeRateResponse): MutableList<String> {
    ...
    fun getExchangeValue(amount: Double): String? {
        //convert amount in c1 to euro
        var euro = amount * currencyStringsMap[currency1]!!
        //convert euro to c2
        euro /= currencyStringsMap[currency2]!!
